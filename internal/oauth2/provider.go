...
func NewProvider(ctx context.Context, pool *pgxpool.Pool, issuerURL string, masterKeyHex string) (*Provider, error) {
	...
	privKey, kid, err := getOrCreateSigningKey(ctx, pool)
	if err != nil {
...
func getOrCreateSigningKey(ctx context.Context, pool *pgxpool.Pool) (*rsa.PrivateKey, string, error) {
	var privPEM, kid string
	err := pool.QueryRow(ctx, `
		SELECT kid, private_key_pem FROM oauth2_signing_keys WHERE id = 'default'`,
	).Scan(&kid, &privPEM)
	...
	if err == nil {
		...
		log.Info().Str("kid", kid).Msg("Loaded OAuth2 signing key from database")
		return key, kid, nil
	}
	...
	log.Info().Str("kid", kid).Msg("Generated and stored new OAuth2 signing key")
	return key, kid, nil
}
